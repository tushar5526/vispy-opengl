...
class Canvas(app.Canvas):
    ...
    def on_draw(self, event):
        ...
        self._trans1 = (np.array(self.trans1.to_list()).astype(np.float32))
        # reshaping to (m, n) to (1, m*n) to support data input in vispy
        self._trans1 = self._trans1.reshape((1, self._trans1.shape[0] * self._trans1.shape[1]))
        self.program['transform'] = self._trans1
        ...
        # vispy takes numpy array in m * n matrix form
        self._trans2 = (np.array(self.trans2.to_list()).astype(np.float32))
        # reshaping to (m, n) to (1, m*n)
        self._trans2 = self._trans2.reshape((1, self._trans2.shape[0] * self._trans2.shape[1]))
        self.program['transform'] = self._trans2
        self.program.draw('triangles', self.indices)
